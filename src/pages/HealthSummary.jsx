...
import { MoveRight, CalendarRange, CheckCircle2, CircleAlert, TrendingUp, TrendingDown } from "lucide-react";
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div>
        ...
        <p className="mt-2 max-w-2xl text-muted">A concise summary generated from your sample health entries.</p>
      </div>
      ...
      <div className="mt-8 rounded-3xl bg-ink p-6 text-white shadow-soft sm:p-8">
        <div className="flex flex-wrap items-center gap-3 text-xs font-semibold text-white/60">
          <span className="rounded-full bg-white/10 px-3 py-1">DEMO · SIMULATED</span>
          <span className="flex items-center gap-1"><CalendarRange size={14} /> {getDateRange()}</span>
        </div>
          ...
          <div className="mt-5 space-y-4">
            {demoSummary.symptoms.map((x) => (
              <div key={x} className="flex gap-3">
                <CheckCircle2 className="mt-0.5 shrink-0 text-ink" size={18} />
      ...
      <section className="mt-6 rounded-3xl border border-line bg-white p-6">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-3">
          {demoSummary.trends.map((t) => {
            ...
            return (
              <div key={t.label} className="rounded-2xl border border-line p-5">
      ...
      <div className="mt-6 rounded-2xl border border-ink/10 bg-cream p-5">
      ...
      <div className="mt-6 flex justify-end">
        <button onClick={onDoctor} className="btn-primary">
          Prepare doctor visit summary <MoveRight size={17} />
        </button>
      </div>
    </div>
  );
